...
- 如果显式使用远程经理模型，连接失败或超时会直接显示为经理聊天里的错误，不会自动改成本地摘要回答
...
- Codex 员工默认使用设备日常的 `~/.codex` 目录；导入设备上已有的 Codex session 时，会同时保留它的 Codex home，确保后续恢复到原始本地会话目录

## 文本资源

直连会话现在开始支持 `文本资源` 这条链路：

- 人可以在与员工的直连对话里上传文本文件
- Hub 会把正文存到 `RESOURCE_STORAGE_DIR`
- 会话消息里只保留资源卡片和引用，不把大文本直接塞进聊天正文
- 任务会同时记录和这些资源的关联
- 数字员工收到任务时，会按需读取资源头部、尾部或片段，而不是默认吞下整份大文件
- 资源会进入独立治理状态：`使用中 / 已归档 / 待清理`
- 删除会话或任务后，资源不会被直接删掉，而是先转成 `待清理`，方便后续复查和回收

这条能力的目标不是做一个厚重的文件管理中心，而是让 crash log、脚本片段、配置文件、说明文档这类材料可以成为 `任务输入`。
...
RESOURCE_STORAGE_DIR=/absolute/path/to/resources
...
- Hub 会把实时状态快照注入给经理模型；员工在线状态、任务进展、审批和资源事实必须以实时快照或工具结果为准，不能靠历史聊天猜
- 如果远程经理模型连接失败、鉴权失败、额度不足或响应超时，Hub 会把错误写进经理聊天内容，不会自动回退成本地摘要
...
如果你不配置经理层 key，首页里的 `AI经理` 仍然可以显式运行在 `MANAGER_PROVIDER=local` 的本地摘要模式；但一旦配置了 `zhipu`、`openai` 或 `openai-compatible` 这类远程 provider，请把模型不可用视为错误，而不是让系统自动猜一个本地答案。
