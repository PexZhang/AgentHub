...
当前这版已经开始从 `单机多 Agent` 往 `多设备多 Agent` 演进：

- 网页会先按 `设备` 分组，再展示这台设备上的数字员工
...
```bash
HUB_ORIGIN=http://你的Hub地址 \
AGENT_TOKEN=你的AGENT_TOKEN \
...
AGENT_ID=codex-office \
AGENT_NAME="Office Codex" \
AGENT_MODE=codex \
npm run agent
```
...
这样接进来以后，AgentHub 的 UI 会先显示 `设备`，再显示这个设备下的多个数字员工，更符合手机端切换心智。
...
- 给设备和 Agent 增加更清晰的身份、能力标签和任务视图
- 把 `conversation` 和 `task/run` 分离，准备支持多数字员工协作
- 继续打磨手机端的 `设备 -> Agent -> 会话` 导航体验
